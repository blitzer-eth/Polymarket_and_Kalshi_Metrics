...
-- 1. Standardize Polymarket Categories
poly_tags AS (
    WITH base AS (
        SELECT
            condition_id,
        ...
        SELECT
            ...
            CASE
                ...
                WHEN array_position(tags_arr_lc, 'crypto') > 0 
                  ...
                  OR array_position(tags_arr_lc, 'crypto prices') > 0
                ...
                WHEN array_position(tags_arr_lc, 'world') > 0 
                  ...
                  OR array_position(tags_arr_lc, 'portugal') > 0 OR array_position(tags_arr_lc, 'brazil') > 0
                ...
                WHEN array_position(tags_arr_lc, 'entertainment') > 0 
                  ...
                  OR array_position(tags_arr_lc, 'taylor swift') > 0 OR array_position(tags_arr_lc, 'music') > 0
                ...
                WHEN array_position(tags_arr_lc, 'social') > 0 OR array_position(tags_arr_lc, 'culture') > 0
                  OR array_position(tags_arr_lc, 'elon musk') > 0
...
-- 2. Calculate Polymarket Volumes
poly_final AS (
    SELECT
        t.category,
    ...
    JOIN poly_tags t ON CAST(p.condition_id AS VARCHAR) = t.condition_id
...
-- 3. Calculate Kalshi Volumes 
kalshi_final AS (
    SELECT 
        CASE 
            WHEN category IN ('Politics', 'Elections') THEN 'Politics'
            WHEN category = 'Economics' THEN 'Economics'
            WHEN category = 'Crypto' THEN 'Crypto'
            WHEN category = 'Science & Technology' THEN 'Science and Technology'
            ELSE category 
        END AS category,
...
-- 4. Combine and Filter for Top 5
combined_results AS (
    SELECT category, market, total_volume FROM poly_final
    UNION ALL
    SELECT category, market, total_volume FROM kalshi_final
),

top_categories AS (
    SELECT 
        category, 
        SUM(total_volume) as combined_vol
...
JOIN top_categories t ON c.category = t.category
ORDER BY t.combined_vol DESC, c.total_volume DESC;
